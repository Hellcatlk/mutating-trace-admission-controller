...
func buildDeploymentPatch(raw []byte, op v1beta1.Operation) *v1beta1.AdmissionResponse {
	...
	spanContext, err := trace.DecodeSpanContext(deployment.GetAnnotations()[spanContextAnnotationKey])
	if err != nil {
		return &v1beta1.AdmissionResponse{
			Result: &metav1.Status{
				Message: err.Error(),
	...
	if op == v1beta1.Create {
		if deployment.GetAnnotations()[initialTraceIDAnnotationKey] != "" {
			inititalTraceID = deployment.GetAnnotations()[initialTraceIDAnnotationKey]
		} else {
